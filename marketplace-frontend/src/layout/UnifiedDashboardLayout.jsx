...
const UnifiedDashboardLayout = () => {
  ...
  const roleConfig = {
    ...
    professional: {
      ...
      icon: Briefcase,
      notifications: [
        { id: 1, title: 'Nueva solicitud de servicio', time: '5m', type: 'info' },
        { id: 2, title: 'Cliente te calificó con 5 estrellas', time: '1h', type: 'success' },
        { id: 3, title: 'Actualiza tu perfil profesional', time: '2h', type: 'warning' }
      ],
      sidebarItems: [
        {
          title: 'Principal',
          items: [
            ...
            { name: 'Mi Perfil', path: '/professional-dashboard/profile', icon: User },
            { name: 'Servicios', path: '/professional-dashboard/services', icon: Briefcase },
            { name: 'Portfolio', path: '/professional-dashboard/portfolio', icon: Camera },
            { name: 'Disponibilidad', path: '/professional-dashboard/availability', icon: Calendar }
          ]
        },
        {
          title: 'Gestión',
          items: [
            { name: 'Solicitudes', path: '/professional-dashboard/requests', icon: Bell },
            { name: 'Trabajos Activos', path: '/professional-dashboard/active-jobs', icon: Clock },
            { name: 'Historial', path: '/professional-dashboard/history', icon: History },
            { name: 'Calificaciones', path: '/professional-dashboard/reviews', icon: Star }
          ]
        },
        {
          title: 'Finanzas',
          items: [
            { name: 'Ingresos', path: '/professional-dashboard/earnings', icon: DollarSign },
            { name: 'Pagos', path: '/professional-dashboard/payments', icon: CreditCard },
            { name: 'Suscripción', path: '/professional-dashboard/subscription', icon: Award }
          ]
        }
      ]
    },
    customer: {
      ...
      icon: User,
      notifications: [
        { id: 1, title: 'Profesional acepto tu solicitud', time: '10m', type: 'success' },
        { id: 2, title: 'Tu trabajo fue completado', time: '1h', type: 'info' },
        { id: 3, title: 'Califica al profesional', time: '3h', type: 'warning' }
      ],
      sidebarItems: [
        {
          title: 'Principal',
          items: [
            ...
            { name: 'Mis Solicitudes', path: '/customer-dashboard/requests', icon: BookOpen },
            { name: 'Trabajos Activos', path: '/customer-dashboard/active-jobs', icon: Clock }
          ]
        },
        {
          title: 'Historial',
          items: [
            { name: 'Trabajos Completados', path: '/customer-dashboard/completed', icon: History },
            { name: 'Favoritos', path: '/customer-dashboard/favorites', icon: Heart },
            { name: 'Mis Calificaciones', path: '/customer-dashboard/reviews', icon: Star }
          ]
        },
        {
          title: 'Cuenta',
          items: [
            { name: 'Mi Perfil', path: '/customer-dashboard/profile', icon: User },
            { name: 'Pagos', path: '/customer-dashboard/payments', icon: CreditCard },
            { name: 'Configuración', path: '/customer-dashboard/settings', icon: Settings }
          ]
        }
      ]
    }
    ...
    <div className={`min-h-screen ${isDarkMode ? 'dark' : ''}`}>
      <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div className={`fixed inset-y-0 left-0 z-50 w-80 bg-white dark:bg-gray-800 transform ${
          ...
          {/* Sidebar Header */}
          <div className={`flex items-center justify-between h-20 px-6 bg-gradient-to-r ${currentConfig.color}`}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center">
                <currentConfig.icon className="w-6 h-6 text-white" />
              </div>
              <div>
                <h1 className="text-white font-bold text-lg">{currentConfig.title}</h1>
                  ...
                  <div key={sectionIndex}>
                    ...
                    <ul className="space-y-1">
                      {section.items.map((item, itemIndex) => {
                        const isActive = isActiveRoute(item.path, item.activeTab);
                        const linkPath = item.activeTab ? `${item.path}?tab=${item.activeTab}` : item.path;
                        ...
                        return (
                          <li key={itemIndex}>
                            ...
                            >
                              ...
                              <span>{item.name}</span>
                              {item.name === 'Dashboard' && user?.role === 'admin' && (
            ...
            <div className="flex items-center justify-between h-full px-4 lg:px-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="hidden sm:block">
                  <div className="relative">
                    ...
                    <input
                      type="text"
                      placeholder={user?.role === 'customer' ? 'Buscar profesionales...' : 'Buscar...'}
              ...
              <div className="flex items-center gap-2 lg:gap-4">
                ...
                <div className="relative lg:hidden">
                  ...
                  >
                    <div className={`w-8 h-8 bg-gradient-to-r ${currentConfig.color} rounded-full flex items-center justify-center`}>
                      <span className="text-white font-semibold text-sm">
                        {user?.name?.charAt(0) || 'U'}
                      </span>
                    </div>
